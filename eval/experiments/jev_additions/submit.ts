// Submit one model per MI325X with a durable idempotency key and status record.
...
const root='/root/open-jev-experiments/jev-additions-v1';
const plan=JSON.parse(await fs.readFile(root+'/plan.json','utf8'));
const slug=process.argv[2];if(!plan.models[slug])throw Error('Unknown model');
...
const name='jev-additions-'+plan.source_revision.slice(0,7)+'-'+slug+'-v1';
const body={name,computeSku:offer.sku,containerImage:plan.image,networkVolumeMounts:mounts,ephemeralDiskGiB:20,workingDirectory:root,command:['bash','/workspace/open-jev/simple-jev-evals/eval/experiments/jev_additions/run_gpu.sh',slug],timeoutSeconds:25200,retainLogs:true};
