...
```sh
python3 eval/prepare_security_rerank.py security-injection \
  --checkout /path/to/jev-sec-bench --output eval/data/security-injection.jsonl
python3 eval/prepare_security_rerank.py security-injection-no-context \
  --checkout /path/to/jev-sec-bench --output eval/data/security-injection-no-context.jsonl
python3 eval/prepare_security_rerank.py security-code \
...
`python prepare_data.py`. This downloads PhishNChips v5.2, checks the upstream
release hashes and creates `data/emails.jsonl`; it does not call a model.

```sh
python3 eval/prepare_security_rerank.py phishing-verdict \
...
Use `prepare_security_rerank.py passage-rerank --dataset NAME`. Supported datasets:
...
```sh
python3 eval/prepare_security_rerank.py passage-rerank --dataset scifact \
